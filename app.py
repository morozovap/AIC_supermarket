...
app = Flask(__name__)
app.secret_key = 'zlagoda-secret-key-2026'
...
def login():
    ...
    if request.method == 'POST':
        ...
        if emp_data:
            ...
            password_hash = emp_list[-1]
            
            if verify_password(password, password_hash):
                session['id_employee'] = emp_list[0]
                session['role']        = emp_list[4]
                session['name']        = f"{emp_list[1]} {emp_list[2]}"
    ...
    if request.method == 'POST':
        ...
        try:
            for i in range(len(upcs)):
                ...
                if existing_product:
                    current_qty = existing_product[0][1]
